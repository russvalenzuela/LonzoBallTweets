...
LonzoTweets <-searchTwitter("Lonzo Ball -filter:retweets", n=20, lang="en")
LonzoTweets_df <- twListToDF(strip_retweets(LonzoTweets, strip_manual=TRUE,strip_mt=TRUE))
#LonzoTweets_df <-twListToDF(LonzoTweets)

currentTime <-Sys.time()
currentTime <- format(currentTime,tz="America/Los_Angeles")
write.table(LonzoTweets_df, file=paste("mydata2.csv",currentTime),sep=",",row.names=F)


#Remove Emojis
LonzoTweets_df$text <- iconv(LonzoTweets_df$text, "latin1", "ASCII", sub="")


#Create Corpus for LA Tweets
LonzoTweets_corpus <- Corpus(VectorSource(LonzoTweets_df$text))
...
#Sentiment Analysis
sentiments_Lonzo <- calculate_total_presence_sentiment(LonzoTweets_df$text)
